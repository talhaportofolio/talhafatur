...
import { 
  ...
  MapPin,
  Globe,
  FileText, // Import ikon FileText untuk CV
  Sparkles // Import ikon Sparkles untuk Soft Skills
} from 'lucide-react';
...
const CustomCursor = () => {
  ...
  useEffect(() => {
    ...
    // Animation Loop untuk RING (memberikan efek delay halus/physics)
    const animateRing = () => {
      // Linear interpolation (Lerp) untuk efek delay
      // 0.15 adalah kecepatan (semakin kecil semakin lambat/berat)
...
const Portfolio = () => {
  ...
  // State untuk mengontrol Accordion Experience (hanya 1 yang terbuka)
  const [openExperienceIndex, setOpenExperienceIndex] = useState(0); 

  // State untuk Soft Skills yang bisa di-reorder
  ...
  // Handle Scroll untuk Active State & Navbar Background
  useEffect(() => {
    const handleScroll = () => {
  ...
  const handleExperienceToggle = (index) => {
    // Hanya mengubah state buka/tutup tanpa memaksa scroll
    const isOpening = openExperienceIndex !== index;
    setOpenExperienceIndex(isOpening ? index : null);
  };

  return (
    <div className="bg-[#0a0a0a] min-h-screen text-slate-300 font-sans selection:bg-cyan-500/30 selection:text-cyan-200 cursor-none">
      <CustomCursor />

      {/* --- NAVBAR --- */}
      <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-[#0a0a0a]/90 backdrop-blur-md py-4 shadow-2xl border-b border-white/5' : 'py-8 bg-transparent'}`}>
            ...
            {navItems.map((item) => (
              <li key={item.id} className="relative group">
                <button
                  ...
                  className={`text-sm md:text-base font-mono transition-colors duration-300 flex flex-col items-center gap-1 cursor-none 
      ...
      <section id="home" className="min-h-screen flex flex-col justify-center items-center relative overflow-hidden px-4">
        ...
        <div className="text-center z-10 relative max-w-4xl mx-auto mt-16">
          <p className="text-cyan-400 font-mono mb-6 tracking-widest text-sm md:text-base animate-fade-in">HELLO, I AM</p>
          
          <h1 className="text-5xl md:text-7xl lg:text-8xl font-black text-slate-100 tracking-tighter mb-8 leading-tight">
          ...
          <div className="flex flex-col md:flex-row justify-center items-center gap-6">
            ...
            <button 
              ...
              className="group flex items-center gap-2 text-white border border-slate-700 px-6 py-2 rounded-full hover:bg-white hover:text-black transition-all duration-300 cursor-none"
            >
      ...
      {/* --- EXPERTISE SECTION --- */}
      <section id="expertise" className="min-h-screen flex flex-col justify-center pt-40 pb-20 px-6 relative bg-[#0f0f0f]">
            ...
            <div className="flex flex-wrap justify-center gap-4 max-w-4xl mx-auto">
              {softSkills.map((skill, index) => (
                <div
                  ...
                  className="px-6 py-3 bg-[#1e1b4b]/30 border border-purple-500/20 text-purple-200 font-medium rounded-full cursor-grab active:cursor-grabbing hover:bg-[#4c1d95]/40 hover:border-purple-500/50 transition-all hover:scale-105 select-none backdrop-blur-sm shadow-lg hover:shadow-purple-500/10"
                >
                  {skill}
                </div>
              ))}
      ...
      {/* --- PROJECT SECTION --- */}
      <section id="project" className="min-h-screen flex flex-col justify-center pt-40 pb-20 px-6">
        <div className="max-w-6xl mx-auto w-full">
      ...
      {/* --- EXPERIENCE SECTION --- */}
      <section id="experience" className="min-h-screen flex flex-col justify-center pt-40 pb-20 px-6 bg-[#0f0f0f]">
      ...
      {/* --- CONTACT SECTION --- */}
      <section id="contact" className="min-h-screen flex flex-col justify-center pt-40 pb-20 px-6 items-center text-center relative overflow-hidden">
        <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-5"></div>
        
        <div className="z-10">
          <p className="text-cyan-500 font-mono mb-6 tracking-widest">// 05 CONTACT</p>
          <h2 className="text-5xl md:text-7xl font-bold text-white mb-8 tracking-tight">Open for<br/>Internship Opportunities</h2>
          ...
          <div className="flex flex-col md:flex-row gap-6 justify-center">
            <a href="mailto:talhafaturjob@gmail.com" className="flex items-center gap-3 bg-white text-black px-8 py-4 rounded-full font-bold hover:bg-cyan-400 hover:text-white transition-all duration-300 shadow-lg hover:shadow-cyan-500/25 cursor-none">
              <Mail size={20} />
              talhafaturjob@gmail.com
            </a>
            <a href="#" className="flex items-center gap-3 border border-slate-700 text-white px-8 py-4 rounded-full font-bold hover:bg-slate-800 transition-all duration-300 cursor-none">
              <FileText size={20} />
              Download CV
            </a>
          </div>

          <div className="mt-20 flex justify-center w-full mx-auto px-10">
             <div className="text-center">
               <h4 className="text-white font-bold mb-4">Socials</h4>
               <ul className="space-y-2 text-slate-500 text-sm">
                 <li className="hover:text-cyan-400 cursor-none"><a href="https://github.com/talhaportofolio">Github</a></li>
                 <li className="hover:text-cyan-400 cursor-none"><a href="https://instagram.com/talhafatur">Instagram</a></li>
                 <li className="hover:text-cyan-400 cursor-none">Twitter</li>
               </ul>
             </div>
          </div>
        </div>
...
const SocialIcon = ({ icon, href }) => (
  <a href={href} className="text-slate-400 hover:text-white hover:bg-white/10 p-2 rounded-full transition-all duration-300 border border-transparent hover:border-white/10 cursor-none">
    {icon}
  </a>
);
...
const SectionTitle = ({ number, title }) => (
  <div className="flex items-center gap-4 mb-12">
    ...
    <div className="h-[1px] bg-gradient-to-r from-slate-800 to-transparent flex-grow ml-6"></div>
  </div>
);

const ExpertiseCard = ({ icon, title, desc, tags }) => (
  <div className="bg-[#121212] p-8 rounded-2xl border border-white/5 hover:border-cyan-500/30 transition-all group hover:-translate-y-2 hover:shadow-xl hover:shadow-cyan-900/10 cursor-none">
    <div className="mb-6 bg-slate-900/50 w-16 h-16 rounded-xl flex items-center justify-center group-hover:bg-cyan-500/20 transition-colors duration-300">
      {icon}
    </div>
    ...
    <div className="flex flex-wrap gap-2">
      {tags && tags.map((tag, i) => (
        <span key={i} className="text-xs font-mono text-slate-500 bg-slate-900 px-2 py-1 rounded border border-slate-800">
          {tag}
        </span>
      ))}
    </div>
...
const ProjectCard = ({ title, category, image, desc, stack }) => (
  <div className="group relative rounded-xl overflow-hidden bg-white/5 backdrop-blur-md border border-white/10 hover:border-cyan-500/50 transition-all cursor-none h-[400px]">
    <div className="relative h-full w-full">
      ...
      />
      {/* Overlay Gradient yang selalu ada tapi lebih gelap saat hover */}
      ...
      <div className="absolute top-4 left-4">
        <span className="px-3 py-1 text-xs font-bold font-mono text-cyan-400 bg-cyan-900/30 border border-cyan-500/30 rounded-full uppercase tracking-wider backdrop-blur-sm">
          {category}
        </span>
      </div>
      ...
      <div className="absolute bottom-0 left-0 p-8 w-full transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
        ...
        <p className="text-slate-300 text-sm mb-6 line-clamp-3 group-hover:line-clamp-none transition-all duration-500 opacity-80 group-hover:opacity-100">
          {desc}
        </p>
        <div className="flex flex-wrap gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100">
           {stack && stack.map((tech, i) => (
             <span key={i} className="text-xs text-cyan-300 font-mono bg-cyan-900/20 px-2 py-1 rounded border border-cyan-500/20">
               #{tech}
             </span>
           ))}
        </div>
...
// --- COMPONENT EXPERIENCE ITEM BARU (ACCORDION) ---
const ExperienceItem = ({ role, company, period, desc, skills, location, isOpen, onToggle }) => {
  return (
    <div className="mb-4 group">
      ...
      <button
        onClick={onToggle}
        // Menghapus 'sticky top-24 z-30' agar header tidak menempel/bergerak saat discroll
        className={`w-full flex items-center justify-between p-6 transition-all duration-300 text-left border border-white/5 cursor-none relative overflow-hidden ${
          isOpen 
            ? 'bg-[#6b21a8] hover:bg-[#7e22ce] rounded-t-xl' // Warna Ungu saat terbuka (Highlight)
            : 'bg-[#121212] hover:bg-[#1a1a1a] rounded-xl' // Warna Gelap saat tertutup (Sesuai Website)
        }`}
      >
        <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-3 z-10">
          <span className={`text-lg md:text-xl font-bold ${isOpen ? 'text-white' : 'text-slate-200'}`}>
            {role}
          </span>
          <span className={`${isOpen ? 'text-purple-200' : 'text-slate-500'} font-medium`}>@ {company}</span>
        </div>
        ...
        <div className="flex items-center gap-4 z-10">
           ...
           <div className={`p-1 rounded-full transition-all duration-300 ${isOpen ? 'bg-white/10 rotate-180' : 'bg-transparent rotate-0'}`}>
             {isOpen ? <Minus size={20} className="text-white" /> : <Plus size={20} className="text-slate-400 group-hover:text-white" />}
           </div>
        </div>
      </button>
      ...
      >
        <div className="p-8">
           {/* Location Info */}
           <div className="flex items-center gap-6 mb-6 text-sm font-mono text-slate-400">
             {location && (
           ...
           {/* Description Content */}
           <div className="text-slate-300 mb-8 leading-relaxed text-base">
             {desc}
           </div>
           ...
           <div className="flex flex-wrap gap-2 pt-4 border-t border-white/5">
             {skills && skills.map((skill, index) => (
                <span key={index} className="px-3 py-1.5 bg-[#4c1d95]/30 border border-purple-500/30 text-purple-200 text-xs font-bold rounded-full hover:bg-[#4c1d95] transition-colors cursor-none">
                  {skill}
                </span>
             ))}
           </div>
